...
https://stackoverflow.com/questions/20558402/open-zsh-scripts-in-sh-mode-in-emacs
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'sh-mode-hook
            (lambda ()
              (if (string-match "\\.zsh-theme$" buffer-file-name)
                  (sh-set-shell "zsh"))))
#+END_SRC

#+RESULTS:
| (lambda nil (if (string-match \.zsh-theme$ buffer-file-name) (sh-set-shell zsh))) | (lambda nil (if (string-match \.zsh$ buffer-file-name) (sh-set-shell zsh))) | sh-electric-here-document-mode |
...
#+BEGIN_SRC emacs-lisp
  (use-package elpy
    ...
    ;;(elpy-use-ipython) ;; problems with characterset
...
*** virtualenv
To use virtualenv's you need to set the $WORKON_HOME variable, but as I set that in my .bashrc, the desktop shortcut seems to ignore it.
A better alternative might be to try to alter the .desktop file, seeing as that will fix the issue for all environment variables.

=$ pyenv-workon= to pick which one you want, there is also =elpy-config=

https://www.reddit.com/r/Python/comments/4w5d4e/psa_ipython_5_will_break_emacs_heres_how_to_fix_it/

#+BEGIN_SRC emacs-lisp
  (if (equal system-name "anake")
      (setenv "WORKON_HOME" "/home/gns/.virt_env"))
