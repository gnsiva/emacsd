...
#+BEGIN_SRC emacs-lisp
  ...
  (setq to-install
        '(python-mode cl-lib yasnippet jedi auto-complete autopair find-file-in-repository magit auctex htmlize org emacs-eclim  smartscan nxml git-commit))
...
** TODO change C-w behaviour to kill word when not in mark mode
